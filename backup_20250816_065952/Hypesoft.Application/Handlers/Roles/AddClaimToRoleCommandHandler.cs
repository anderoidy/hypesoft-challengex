...
namespace Hypesoft.Application.Handlers.Roles
{
    public class AddClaimToRoleCommandHandler : IRequestHandler<AddClaimToRoleCommand, Result>
    {
        ...
        public async Task<Result> Handle(AddClaimToRoleCommand request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("Adding claim {ClaimType} with value {ClaimValue} to role {RoleId}", 
                    request.ClaimType, request.ClaimValue, request.RoleId);
                ...
                {
                    _logger.LogWarning("Claim {ClaimType} with value {ClaimValue} already exists for role {RoleName}", 
                        request.ClaimType, request.ClaimValue, role.Name);
                ...
                if (!result.Succeeded)
                {
                    ...
                    _logger.LogError("Failed to add claim to role {RoleName}: {Errors}", role.Name, errors);
                    return Result.Error($"Failed to add claim to role: {errors}");
                }

                _logger.LogInformation("Successfully added claim {ClaimType} to role {RoleName}", 
                    request.ClaimType, role.Name);
                return Result.Success();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error adding claim to role {RoleId}: {ErrorMessage}", 
