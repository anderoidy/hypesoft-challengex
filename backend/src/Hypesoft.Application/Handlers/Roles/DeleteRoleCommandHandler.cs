    ...
    public class DeleteRoleCommandHandler : IRequestHandler<DeleteRoleCommand, Result>
    {
        ...
        )
        {
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "Cannot delete role {RoleName} because it has {UserCount} users assigned",
                        role.Name,
                        usersInRole.Count
                    );
                    return Result.Error(
                        "Cannot delete role because it has users assigned. Please reassign or remove users first."
                    );
                }
            ...
            catch (DbUpdateException ex)
            {
                _logger.LogError(
                    ex,
                    "Database error while deleting role with ID {RoleId}",
                    request.Id
                );
                return Result.Error(
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error deleting role with ID {RoleId}: {ErrorMessage}",
                    request.Id,
                    ex.Message
                );
                return Result.Error(
                    $"An unexpected error occurred while deleting the role: {ex.Message}"
                );
            }
        }
    }
